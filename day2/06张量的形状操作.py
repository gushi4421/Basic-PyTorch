...
涉及到的API:
    ...
    unsqueeze():在指定的轴上一个维度,等价与:升维
    transpose():一次只能交换2个维度
    permute():一个可以同时交换多个维度
    view():只能修改连续的张量, 连续张量=内存中存储顺序和在张量中显示的顺序相同
    contiguous():把不连续的张量 -> 连续的张量,即:基于张量显示的顺序,修改内存中的存储结构
...
def demo01():
    ...
    # 3. 尝试通过reshape()函数,把t1->2行5列的结果   ##报错
    # t3 = t1.reshape(2, 5)
    # print(f"t3:{t3}")
...
def demo03():
    ...
    # 3. 改变维度从(2,3,4)->(4,2,3)
    # t3 = t1.transpose(0, 1).transpose(0, -1)
    t3 = t1.permute(2, 0, 1)
...
def demo04():
    ...
    # 5. 尝试把t3张量 从 (3,2) ,通过 view() 转成 (2,3)
    # t4 = t3.view(2, 3)  # t3不连续，报错
    # print(f"t4:{t4},shape:{t4.shape}")

    # 6. 可以通过contiguous()函数,把t3张量->连续张量->然后就能通过view()转为(2,3)
    t4 = t3.contiguous().view(3, 2)
    print(f"t4:{t4},shape:{t4.shape}")
