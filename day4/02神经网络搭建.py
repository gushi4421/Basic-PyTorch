...
class ModuleDemo(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # 1.3 第三层 输出层计算: 加权求和 + 激活函数(Softmax)
        # dim=-1表示按行计算  dim=0表示按列进行计算
...
def train():
    ...
    # 3. 调用神经网络模型 -> 进行模型训练
    output = my_model(data)  # 底层自动调用了forward()方法,进行 前向传播
    print(f"output:{output}")
    print(f"output.shape:{output.shape}")  # (5行,3列)
